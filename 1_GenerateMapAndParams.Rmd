...
#sites that are the farthest north, south, east and west
borders=sites_lonlat[c(1,15,19,33),]

# points 16.4km N,S,E,W of those (respectively)
bord_n <- destPoint(borders[1,],0,16.4*1000)
bord_s <- destPoint(borders[3,],180,16.4*1000)
bord_w <- destPoint(borders[2,],270,16.4*1000)
bord_e <- destPoint(borders[4,],90,16.4*1000)
...
ggsave(filename="map_SLiM.png",plot=g,width=round(dist_x)/10,height=round(dist_y)/10,units='cm')
...
# that includes Glover's and a bunch of lagoon, so I'm cropping those out: xmax=-88 to exclude Glover's, xmin=-88.08 to exclude lagoon
### !! This is a pretty rough method and I don't like it, but I'll go with it for now. A better map would help.
parentage_transect <- st_crop(reef_sim,xmin=-88.08,xmax=-88,ymin=transect_s[2],ymax=transect_n[2])
...
area_reef <- sum(st_area(reef_sim))
units(area_reef) <- "km^2"
#Looking at a map, ~701km^2 (a box 26km on each side) seems about right for the total reef area

#--------------------------------------------------------------------
# 3. Total number of individuals on the reef 
#--------------------------------------------------------------------
...
sgs_sites_slim <- data.frame(x = rep(NA,nrow(sites_lonlat)), y=rep(NA,nrow(sites_lonlat)))

for(i in 1:nrow(sites_lonlat)){
  sgs_sites_slim$x[i] = distGeo(sites_lonlat[i,],c(min_lon,sites_lonlat[i,2]))
  sgs_sites_slim$y[i] = distGeo(sites_lonlat[i,],c(sites_lonlat[i,1],min_lat))
}

sgs_sites_slim=sgs_sites_slim/1000 # convert to km
