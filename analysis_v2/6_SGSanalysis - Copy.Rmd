...
fst_summary_dists <- left_join(fst_summary,sitepair_dists,by = 'sites', keep=FALSE) %>%
  ...
  filter(both_regions %in% c('NN','CC','CN'))
...
fst_dists <- left_join(filter(fst_all_sim,tick>2500),dplyr::select(sitepair_dists, sites,dist),by = 'sites', keep=FALSE) %>%
  mutate(scale_sim = fst/(1-fst)) %>%
  left_join(fst_emp,by='sites') %>%
  mutate(scale_emp = fst.y/(1-fst.y)) %>%
  dplyr::select(tick, sites,site1,site2,scale_sim,scale_emp,dist, both_pops,both_regions) %>%
...
ticks <- unique(fst_dists$tick)
ibd_slopes_sim <- data.frame(ibd_slope=numeric(), slope_se=numeric(),mantel_p=numeric(),mantel_r=numeric())

for(t in 1:length(ticks)){
  fst_dists_t <- filter(fst_dists,tick==ticks[t])
  
  ## build physical and genetic distance matrices
  if(t==1){
  ...
  ## store it all  
  ibd_slopes_sim[t,] <- c(slope,se,p_val,r_val)
} #t
...
## get confidence intervals around the slope (bootstrap)
slopes=vector()
for(rep in 1:100){
  ...
  summ <- summary(ibd_model_rep)
  slopes[rep] <- summ$coefficients[2,1]
}

l_boot <- quantile(slopes,probs=0.025)
u_boot <- quantile(slopes,probs=0.975)
hist(slopes)
quantile(slopes,probs=c(0.025,0.975))

## get confidence intervals around the slope (parametric)
t_star <- qt(0.975,df=nrow(fst_dists_t)-2)
ibd_slopes_sim <- ibd_slopes_sim %>%
  mutate(l_param=ibd_slope-t_star*slope_se, u_param=ibd_slope+t_star*slope_se)
```
